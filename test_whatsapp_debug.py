...
def test_twilio_connection():
    ...
    try:
        ...
        print("✅ Twilio client initialization successful")
        return True
...
def test_webhook_handler():
    ...
    try:
        ...
        if response.status_code == 200:
            print("✅ Test endpoint accessible at http://localhost:8000/api/webhooks/test")
        else:
            print("❌ Test endpoint not returning 200")
    except Exception as e:
...
def check_env_vars():
    ...
    for var in required_vars:
        value = os.getenv(var)
        if value:
            print(f"✅ {var}: {'*' * min(10, len(value)) if 'TOKEN' in var or 'SID' in var else value}")
        else:
            print(f"❌ {var}: NOT SET")
            all_present = False
...
def main():
    ...
    if not twilio_ok:
        print("\n❌ Please fix Twilio connection issues before testing webhook!")
        return
